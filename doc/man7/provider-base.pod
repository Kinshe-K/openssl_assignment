...
L<OSSL_DISPATCH(3)> arrays consist of entries as a function pairs with
unique function identifiers and pointers as defined in 
L<openssl-core_dispatch.h(7)>. These are searched where the function ID
matches a desired macro to be cast to the appropriate function type,
terminating in empty id/pointer pair of {0, NULL}, as follows:
